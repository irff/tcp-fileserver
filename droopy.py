    ...
    def html(self, page):
        ...
        if self.publish_files:
            for name in self.published_files():
                encoded_name = urllibparse.quote(name)
    ...
    def do_POST(self):
        ...
        try:
            ...
            for item in file_items:
                ...
                obj = os.path.basename(localpath)
                # start presentation
...
linkurltmpl = '''<div id="linkurl" class="box">PROYEKTOR AJAIB</div>'''
...
default_localisations = {
    'en' : {
        "maintitle":       "Kirim dokumen",
        "submit":          "Kirim",
        "sending":         "Mengirim",
        "successtitle":    "Dokumen diterima",
        "received":        "Dokumen diterima!",
        "another":         "Kirim dokumen lain.",
        "errortitle":      "Ada masalah",
        "problem":         "Ada masalah!",
        "retry":           "Ulangi.",
        "discover":        "Apa alamat web ini"}
